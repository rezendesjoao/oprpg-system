...
export async function migrateWorld({ bypassVersionCheck=false }={}) {
  ...
  game.settings.set("onepiece-system", "systemMigrationVersion", game.system.version);
...
export async function migrateSettings() {
  ...
  if ( (disableExperienceTracking !== undefined) && (levelingMode === undefined) ) {
    await game.settings.set("onepiece-system", "levelingMode", "noxp");
  }
  ...
  if ( (disableMovementAutomation !== undefined) && (movementAutomation === undefined) ) {
    await game.settings.set("onepiece-system", "movementAutomation", disableMovementAutomation ? "none" : "full");
  }
}
...
export function migrateItemData(item, itemData, migrationData, flags={}) {
  ...
  // Migrate embedded effects
  if ( itemData.effects ) {
    const riders = foundry.utils.getProperty(itemData, "flags.OnePiece.riders.effect");
    if ( riders?.length ) updateData["flags.OnePiece.riders.effect"] = riders;
    const effects = migrateEffects(itemData, migrationData, updateData, flags);
    if ( riders?.length === updateData["flags.OnePiece.riders.effect"]?.length ) {
      delete updateData["flags.OnePiece.riders.effect"];
    }
    if ( effects.length > 0 ) updateData.effects = effects;
  }
  ...
  // Migrate properties
  const migratedProperties = foundry.utils.getProperty(itemData, "flags.OnePiece.migratedProperties");
  if ( migratedProperties?.length ) {
    ...
    updateData["flags.OnePiece.-=migratedProperties"] = null;
  }
  ...
  if ( (itemData.type === "spell") && !itemData.system?.sourceItem && flags.actorData?.items ) {
    ...
    if ( !grantingItemData ) {
      const cachedFor = item.getFlag("onepiece-system", "cachedFor");
      if ( cachedFor ) {
  ...
  if ( foundry.utils.getProperty(itemData, "flags.OnePiece.persistSourceMigration") ) {
    flags.persistSourceMigration = true;
    updateData["flags.OnePiece.-=persistSourceMigration"] = null;
  }
...
export function migrateEffects(parent, migrationData, itemUpdateData, flags={}) {
  ...
  return parent.effects.reduce((arr, e) => {
    ...
    if ( effectData.flags?.dnd5e?.rider ) {
      itemUpdateData["flags.OnePiece.riders.effect"] ??= [];
      itemUpdateData["flags.OnePiece.riders.effect"].push(effectData._id);
      effectUpdate["flags.OnePiece.-=rider"] = null;
    }
    if ( effectData.flags?.dnd5e?.persistSourceMigration ) {
      flags.persistSourceMigration = true;
      effectUpdate["flags.OnePiece.-=persistSourceMigration"] = null;
    }
...
export function migrateMessageData(messageData) {
  ...
  if ( (flags?.dnd5e?.messageType === "usage") && (messageData.type !== "usage") ) {
    ...
    updateData["flags.OnePiece.-=messageType"] = null;
    updateData["flags.OnePiece.-=scaling"] = null;
    updateData["flags.OnePiece.use.-=cause"] = null;
    updateData["flags.OnePiece.use.-=concentrationId"] = null;
    updateData["flags.OnePiece.use.-=consumed"] = null;
    updateData["flags.OnePiece.use.-=effects"] = null;
    updateData["flags.OnePiece.use.-=spellLevel"] = null;
  }

  else if ( flags?.dnd5e?.bastion && (messageData.type === "base") ) {
    const bastion = flags.OnePiece.bastion;
    updateData.type = "orders" in bastion ? "bastionTurn" : "bastionAttack";
    updateData["==system"] = bastion;
    updateData["flags.OnePiece.-=bastion"] = null;
  }
...
export async function getMigrationData() {
  const data = {};
  try {
    const icons = await fetch("systems/onepiece-system/json/icon-migration.json");
    const spellIcons = await fetch("systems/onepiece-system/json/spell-icon-migration.json");
...
function _migrateActorFlags(actorData, updateData) {
  const initiativeAdv = foundry.utils.getProperty(actorData, "flags.OnePiece.initiativeAdv");
  if ( initiativeAdv ) {
    const key = "system.attributes.init.roll.mode";
    updateData[key] = Math.min(1, (foundry.utils.getProperty(actorData, key) ?? 0) + 1);
    updateData["flags.OnePiece.-=initiativeAdv"] = null;
  }
  return updateData;
}
...
function _migrateTokenImage(actorData, updateData) {
  ...
  for ( const path of ["texture.src", "prototypeToken.texture.src"] ) {
    ...
    if ( oldSystemPNG.test(v) ) {
      ...
      updateData[path] = `systems/onepiece-system/tokens/${type}/${fileName}.webp`;
    }
  }
  return updateData;
}
...
function _migrateItemUses(item, itemData, updateData, flags) {
  const value = foundry.utils.getProperty(itemData, "flags.OnePiece.migratedUses");
  ...
  if ( value !== undefined ) updateData["flags.OnePiece.-=migratedUses"] = null;
}
...
export async function purgeFlags(pack) {
  const cleanFlags = flags => {
    const flags5e = flags.OnePiece || null;
