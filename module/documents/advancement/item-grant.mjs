...
export default class ItemGrantAdvancement extends Advancement {
  ...
  static get metadata() {
    return foundry.utils.mergeObject(super.metadata, {
      ...
      typeIcon: "systems/onepiece-system/icons/svg/item-grant.svg",
  ...
  restore(level, data, options={}) {
    ...
    for ( const item of data.items ?? [] ) {
      ...
      updates[item._id] = item.flags.OnePiece.sourceId;
    }
    this.updateSource({
  ...
  reverse(level, options={}) {
    ...
    for ( const id of ids ) {
      ...
      if ( item ) {
        ...
        items[item.flags.OnePiece?.sourceId ?? item._stats.compendiumSource ?? item.uuid] = item.toObject();
      }
