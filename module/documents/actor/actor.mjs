...
export default class Actor5e extends SystemDocumentMixin(Actor) {
  ...
  /** @override */
  static DEFAULT_ICON = "systems/onepiece-system/icons/svg/documents/actor.svg";
   ...
   */
  get isPolymorphed() {
    return this.getFlag("onepiece-system", "isPolymorphed") || false;
  }
  ...
  get concentration() {
    ...
    for ( const effect of this.effects ) {
      ...
      const data = effect.getFlag("onepiece-system", "item");
  ...
  _initializeSource(source, options={}) {
    ...
    if ( (source.type === "group") && (source.system?.type?.value === "encounter") ) {
      ...
      foundry.utils.setProperty(source, "flags.OnePiece.persistSourceMigration", true);
    }
  ...
  *allApplicableEffects() {
    for ( const effect of super.allApplicableEffects() ) {
      ...
      if ( effect.parent?.getFlag("onepiece-system", "riders.effect")?.includes(effect.id) ) continue;
      yield effect;
    }
  }
  ...
  static async fetchExisting(uuid, options={}) {
    ...
    const localActor = game.actors.find(a => {
      ...
      return (a.getFlag("onepiece-system", "isAutoImported") || a.getFlag("onepiece-system", "summonedCopy")) // Back-compat
    ...
    if ( actor.pack ) {
      ...
      return game.actors.importFromCompendium(game.packs.get(actor.pack), actor.id, {
        "flags.OnePiece.isAutoImported": true
      });
    } else {
  ...
  async getPreferredArtwork() {
    if ( !this._preferredArtwork ) {
      const showTokenPortrait = this.getFlag("onepiece-system", "showTokenPortrait") === true;
  ...
  /** @inheritDoc */
  prepareDerivedData() {
    const origin = this.getFlag("onepiece-system", "summon.origin");
  ...
  calculateDamage(damages, options={}) {
    ...
    const treatAs = options.originatingMessage?.flags?.dnd5e?.roll?.type
      ? options.originatingMessage.flags.OnePiece.roll.type === "healing" ? "healing" : "damage"
      : options.only ?? "damage";
  ...
  async endConcentration(target) {
    ...
    if ( foundry.utils.getType(target) === "string" ) effect = effects.find(e => e.id === target);
      ...
      effect = effects.find(e => {
        const data = e.getFlag("onepiece-system", "item") ?? {};
  ...
  async challengeConcentration({ dc=10, ability=null }={}) {
    ...
    return ChatMessage.implementation.create({
      content: await foundry.applications.handlebars.renderTemplate(
        "systems/onepiece-system/templates/chat/roll-request-card.hbs",
        {
          buttons: [{
  ...
  _isRemarkableAthlete(ability) {
    return (dnd5e.settings.rulesVersion === "legacy") && this.getFlag("onepiece-system", "remarkableAthlete")
  ...
  static async handleSkillCheckRequest(actor, request, config, { event }={}) {
    const data = {};
    foundry.utils.setProperty(data, "flags.OnePiece.requestResult", { actorUuid: actor.uuid, requestId: request.id });
  ...
  async #rollSkillTool(type, config={}, dialog={}, message={}) {
    ...
    const hostActor = this.isPolymorphed && this.flags?.dnd5e?.transformOptions?.mergeSkills && (type === "skill")
      ? game.actors.get(this.flags.OnePiece?.originalActor) : null;
    ...
    const rollConfig = foundry.utils.mergeObject({
      ...
      halflingLucky: this.getFlag("onepiece-system", "halflingLucky"),
      reliableTalent: (relevant?.value >= 1) && this.getFlag("onepiece-system", "reliableTalent")
    }, config);
  ...
  async rollDeathSave(config={}, dialog={}, message={}) {
    ...
    if ( this.getFlag("onepiece-system", "diamondSoul") ) {
      parts.push("@prof");
  ...
  getInitiativeRollConfig(options={}) {
    ...
    const flags = this.flags.OnePiece ?? {};
    ...
    const tiebreaker = game.settings.get("onepiece-system", "initiativeDexTiebreaker");
    if ( tiebreaker && Number.isNumeric(ability?.value) ) parts.push(String(ability.value / 100));

    // Fixed initiative score
    const scoreMode = game.settings.get("onepiece-system", "initiativeScore");
  ...
  async rollHitDie(config={}, dialog={}, message={}) {
    ...
    const messageConfig = foundry.utils.mergeObject({
      ...
      data: {
        ...
        "flags.OnePiece.roll": {type: "hitDie"}
      }
    }, message);
  ...
  async rollClassHitPoints(item, { chatMessage=true }={}) {
    ...
    const messageData = {
      ...
      flavor,
      speaker: ChatMessage.implementation.getSpeaker({ actor: this }),
      "flags.OnePiece.roll": { type: "hitPoints" }
    };
  ...
  async initiateRest(config={}) {
    ...
    if ( !game.user.isGM && !game.settings.get("onepiece-system", "allowRests") && !config.request ) {
    ...
    config = foundry.utils.mergeObject({
      ...
      duration: restConfig.duration[game.settings.get("onepiece-system", "restVariant")],
    ...
    if ( config.autoEnergyED ) await this.autoSpendEnergyDice({ threshold: config.autoHDThreshold });
  ...
  async _rest(config, result={}) {
    ...
    await this._getRestEnergyDiceRecovery(config, result);

    // One Piece: descanso longo restaura todos os Pontos de Poder (PP) ao máximo.
    if ( (config.type === "long") && (this.system.energy?.max != null) ) {
      result.updateData["system.energy.total"] = this.system.energy.max;
    }
    ...
    if ( config.advanceBastionTurn && game.user.isGM && game.settings.get("onepiece-system", "bastionConfiguration").enabled
  ...
  async _displayRestResultMessage(config, result) {
    ...
    if ( config.type === "short" && result.energyRecovered > 0 ) {
      energyLine = ` Recuperou também <strong>${result.energyRecovered} PP</strong> de Pontos de Poder (${result.energyDiceSpent}× ${result.energyDenomination}).`;
    }
    ...
    if ( config.request ) foundry.utils.setProperty(chatData, "flags.OnePiece.requestResult", {
    ...
    if ( rolls.length ) {
      ...
      for ( const roll of rolls ) {
        await roll.toMessage({
          speaker: ChatMessage.getSpeaker({ actor: this, alias: this.name }),
          flavor: `${this.name} recupera PA de Pontos de Poder (${ed.denomination} + mod. CON)`
        });
      }
    }
  ...
  /**
   * Recovers energy dice during a long rest.
   * Amount recovered = actor level. Energy does NOT recover automatically.
   * @param {RestConfiguration} [config={}]
   * @param {RestResult} [result={}]
   * @protected
   */
  async _getRestEnergyDiceRecovery(config={}, result={}) {
    ...
    const recovered = Math.min(level, max - current);
    if ( recovered <= 0 ) return;

    result.updateData ??= {};
    result.updateData["system.energyDice.value"] = current + recovered;
  ...
  getOriginalStats() {
    ...
    if ( this.isPolymorphed ) {
      const transformOptions = this.flags.OnePiece?.transformOptions;
      const original = game.actors?.get(this.flags.OnePiece?.originalActor);
      if ( original ) {
  ...
  async transformInto(source, settings=new TransformationSetting(), options={}) {
    ...
    const allowed = game.settings.get("onepiece-system", "allowPolymorphing");
    ...
    o.flags["onepiece-system"] = o.flags["onepiece-system"] || {};
    o.flags["onepiece-system"].transformOptions = {
      ...settings.toObject(),
    ...
    // Set new data flags
    if ( !this.isPolymorphed || !d.flags.OnePiece.originalActor ) d.flags.OnePiece.originalActor = this.id;
    d.flags.OnePiece.isPolymorphed = true;

    // Gather previous actor data
    const previousActorIds = this.getFlag("onepiece-system", "previousActorIds") || [];
    ...
    if ( this.isToken ) {
      ...
      if ( !this.token.flags.OnePiece?.previousActorData ) {
        ...
        foundry.utils.setProperty(tokenData, "flags.OnePiece.previousActorData", previousActorData);
        foundry.utils.setProperty(tokenData, "flags.OnePiece.previousTokenData", previousTokenData);
      }
    ...
    const updates = tokens.map(t => {
      ...
      const dOriginalActor = foundry.utils.getProperty(d, "flags.OnePiece.originalActor");
      foundry.utils.setProperty(newTokenData, "flags.OnePiece.originalActor", dOriginalActor);
      foundry.utils.setProperty(newTokenData, "flags.OnePiece.isPolymorphed", true);
      if ( !t.document.flags.OnePiece?.previousTokenData ) {
        const previousTokenData = { texture: {} };
        for ( const k of [...tokenPropsFromSource, ...tokenPropsFromSelf, "name"] ) {
        ...
        foundry.utils.setProperty(newTokenData, "flags.OnePiece.previousTokenData", previousTokenData);
      }
      return newTokenData;
    });
  ...
  async revertOriginalForm(options={}) {
    ...
    const transformOptions = this.getFlag("onepiece-system", "transformOptions");
    const previousActorIds = this.getFlag("onepiece-system", "previousActorIds") ?? [];
    const isRendered = this.sheet.rendered;

    // Obtain a reference to the original actor
    const original = game.actors.get(this.getFlag("onepiece-system", "originalActor"));
    ...
    if ( this.isToken ) {
      ...
      if ( !baseActor ) {
        ui.notifications.warn(game.i18n.format("DND5E.TRANSFORM.Warning.OriginalActor", {
          reference: this.getFlag("onepiece-system", "originalActor")
        }));
        return;
      }
      const prototypeTokenData = (await baseActor.getTokenDocument()).toObject();
      const actorData = this.token.getFlag("onepiece-system", "previousActorData");
      ...
      foundry.utils.mergeObject(tokenUpdate, this.token.getFlag("onepiece-system", "previousTokenData"));
      tokenUpdate.sight = prototypeTokenData.sight;
      tokenUpdate.detectionModes = prototypeTokenData.detectionModes;
      delete tokenUpdate.flags.OnePiece.previousActorData;
      delete tokenUpdate.flags.OnePiece.previousTokenData;
    ...
    if ( canvas.ready ) {
      ...
      const tokenUpdates = tokens.map(t => {
        ...
        foundry.utils.mergeObject(update, t.document.getFlag("onepiece-system", "previousTokenData"));
        delete update.x;
        delete update.y;
        return update;
      });
    ...
    if ( game.user.isGM ) {
      ...
      const actorUpdates = polymorphedActorIds.filter(id => game.actors.get(id).isOwner).map(p => {
        return { _id: p, "flags.OnePiece": { "-=isPolymorphed": null, "-=previousActorIds": null } };
      });
  ...
  static addDirectoryContextOptions(app, entryOptions) {
    ...
    entryOptions.push({
      ...
      callback: li => {
        game.settings.set("onepiece-system", "primaryParty", { actor: game.actors.get(li.dataset.documentId ?? li.dataset.entryId) });
      },
      condition: li => {
        const actor = game.actors.get(li.dataset.documentId ?? li.dataset.entryId);
      ...
      callback: li => {
        game.settings.set("onepiece-system", "primaryParty", { actor: null });
      },
      condition: li => {
        const actor = game.actors.get(li.dataset.documentId ?? li.dataset.entryId);
...
async _preUpdate(changed, options, user) {
  ...
  async _onUpdateExhaustion(data, options) {
    ...
    if ( level < 1 ) return effect?.delete();
      ...
      return effect.update({ "flags.OnePiece.exhaustionLevel": level }, { dnd5e: { originalExhaustion } });
    } else {
      effect = await ActiveEffect.implementation.fromStatusEffect("exhaustion", { parent: this });
      effect.updateSource({ "flags.OnePiece.exhaustionLevel": level });
  ...
  updateBloodied(options) {
    const hp = this.system.attributes?.hp;
    if ( !hp?.effectiveMax || (game.settings.get("onepiece-system", "bloodied") === "none") ) return;
  ...
  updateEncumbrance(options) {
    ...
    if ( !encumbrance || (game.settings.get("onepiece-system", "encumbrance") === "none") ) return;
    const statuses = [];
    const variant = game.settings.get("onepiece-system", "encumbrance") === "variant";
