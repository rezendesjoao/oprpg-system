...
export default class ChatMessage5e extends ChatMessage {
   ...
   */
  get canApplyDamage() {
    const type = this.flags.OnePiece?.roll?.type;
  ...
  get canSelectTargets() {
    if ( this.flags.OnePiece?.roll?.type !== "attack" ) return false;
  ...
  get isRoll() {
    ...
    return super.isRoll && !this.flags.OnePiece?.rest;
  }
  ...
  get shouldDisplayChallenge() {
    ...
    switch ( game.settings.get("onepiece-system", "challengeVisibility") ) {
  ...
  static migrateData(source) {
    ...
    if ( foundry.utils.hasProperty(source, "flags.OnePiece.itemData") ) {
      foundry.utils.setProperty(source, "flags.OnePiece.item.data", source.flags.OnePiece.itemData);
      delete source.flags.OnePiece.itemData;
    }
    if ( foundry.utils.hasProperty(source, "flags.OnePiece.use") ) {
      const use = source.flags.OnePiece.use;
      if ( source.type !== "usage" ) foundry.utils.setProperty(source, "flags.OnePiece.messageType", "usage");
      if ( use.type ) foundry.utils.setProperty(source, "flags.OnePiece.item.type", use.type);
      if ( use.itemId ) foundry.utils.setProperty(source, "flags.OnePiece.item.id", use.itemId);
      if ( use.itemUuid ) foundry.utils.setProperty(source, "flags.OnePiece.item.uuid", use.itemUuid);
    }
    return source;
  }
  ...
  prepareData() {
    super.prepareData();
    if ( !this.flags.OnePiece?.item?.data && this.flags.OnePiece?.item?.id ) {
      const itemData = this.system.deltas?.deleted?.find(i => i._id === this.flags.OnePiece.item.id);
      if ( itemData ) Object.defineProperty(this.flags.OnePiece.item, "data", { value: itemData });
    }
  ...
  async renderHTML(options={}) {
    ...
    if ( foundry.utils.getType(this.system?.getHTML) === "function" ) {
      ...
      if ( game.settings.get("onepiece-system", "autoCollapseItemCards") ) {
  ...
  _collapseTrays(html) {
    let collapse;
    switch ( game.settings.get("onepiece-system", "autoCollapseChatTrays") ) {
  ...
  _highlightCriticalSuccessFailure(html) {
    ...
    const displayAttackResult = game.user.isGM || (game.settings.get("onepiece-system", "attackRollVisibility") !== "none");
    const forceSuccess = this.flags.OnePiece?.roll?.forceSuccess === true;
    ...
    for ( let [index, d20Roll] of this.rolls.entries() ) {
      ...
      const canCrit = ["attack", "death"].includes(this.getFlag("onepiece-system", "roll.type"));
      const isAttack = this.getFlag("onepiece-system", "roll.type") === "attack";
  ...
  async _enrichChatCard(html) {
    ...
    const roll = this.getFlag("onepiece-system", "roll");
  ...
  _enrichAttackTargets(html) {
    ...
    const visibility = game.settings.get("onepiece-system", "attackRollVisibility");
    const isVisible = game.user.isGM || (visibility !== "none");
    if ( !isVisible ) return;

    const targets = this.getFlag("onepiece-system", "targets");
  ...
  _enrichDamageTooltip(rolls, html) {
    ...
    const damageOnSave = this.getFlag("onepiece-system", "roll.damageOnSave");
    if ( damageOnSave ) {
      const p = document.createElement("p");
      p.classList.add("supplement");
  ...
  getAssociatedActivity() {
    const activity = fromUuidSync(this.getFlag("onepiece-system", "activity.uuid"), { strict: false });
    if ( activity ) return activity;
    return this.getAssociatedItem()?.system.activities?.get(this.getFlag("onepiece-system", "activity.id"));
  }
   ...
   */
  getAssociatedItem() {
    const item = fromUuidSync(this.getFlag("onepiece-system", "item.uuid"), { strict: false });
    if ( item ) return item;
    const actor = this.getAssociatedActor();
    if ( !actor ) return;
    const storedData = this.getFlag("onepiece-system", "item.data") ?? this.getOriginatingMessage().getFlag("onepiece-system", "item.data");
  ...
  getOriginatingMessage() {
    return game.messages.get(this.getFlag("onepiece-system", "originatingMessage")) ?? this;
  }
}
