...
export default class Item5e extends SystemDocumentMixin(Item) {
  ...
  /** @override */
  static DEFAULT_ICON = "systems/onepiece-system/icons/svg/documents/item.svg";
   ...
   */
  get canDelete() {
    return !this.flags.OnePiece?.cachedFor;
  }
   ...
   */
  get dependentOrigin() {
    return fromUuidSync(this.flags.OnePiece?.dependentOn, { relative: this, strict: false }) ?? null;
  }
   ...
   */
  get scalingIncrease() {
    return this.system?.scalingIncrease ?? this.getFlag("onepiece-system", "scaling") ?? 0;
  }
  ...
  *allApplicableEffects() {
    ...
    const messageConfig = foundry.utils.mergeObject({
      ...
      data: {
        content: await foundry.applications.handlebars.renderTemplate(
          "systems/onepiece-system/templates/chat/item-card.hbs", context
        ),
        flags: {
  ...
  async deleteDialog({ sheet, ...dialogOptions }={}, operation={}) {
    ...
    if ( this.actor?.system.metadata?.supportsAdvancement && !game.settings.get("onepiece-system", "disableAdvancements") ) {
  ...
  static async createScrollFromSpell(spell, options={}, config={}) {
    ...
    config = foundry.utils.mergeObject({
      explanation: game.user.getFlag("onepiece-system", "creation.scrollExplanation") ?? "reference",
    ...
    if ( config.dialog !== false ) {
      ...
      await game.user.setFlag("onepiece-system", "creation.scrollExplanation", config.explanation);
    }
    ...
    if ( Number.isNumeric(config.level) ) {
      flags.OnePiece ??= {};
      flags.OnePiece.scaling = Math.max(0, config.level - spell.system.level);
      flags.OnePiece.spellLevel = {
        value: config.level,
