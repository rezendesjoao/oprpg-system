...
export default class ActiveEffect5e extends DependentDocumentMixin(ActiveEffect) {
   ...
   * @type {string}
   */
  static DEFAULT_ICON = "systems/onepiece-system/icons/svg/documents/active-effect.svg";
  ...
  get dependentOrigin() {
    ...
    return this.parent.effects.get(this.flags.OnePiece?.dependentOn) ?? null;
  }
  ...
  get isConcealed() {
    ...
    if ( (this.id === this.constructor.ID.BLOODIED) && (game.settings.get("onepiece-system", "bloodied") === "player") ) {
  ...
  /** @inheritDoc */
  get isTemporary() {
    return !this.isConcealed && (super.isTemporary || this.getFlag("onepiece-system", "isTemporary"));
  }
  ...
  _initializeSource(data, options={}) {
    ...
    if ( data.flags?.dnd5e?.type === "enchantment" ) {
      ...
      delete data.flags.OnePiece.type;
      foundry.utils.setProperty(data, "flags.OnePiece.persistSourceMigration", true);
    }
  ...
  static migrateData(data) {
    ...
    for ( const change of data.changes ?? [] ) {
      if ( change.key === "flags.OnePiece.initiativeAdv" ) {
  ...
  apply(doc, change) {
    ...
    if ( change.key.startsWith("flags.OnePiece.") ) change = this._prepareFlagChange(doc, change);
  ...
  static applyChange(model, change, options={}) {
    ...
    if ( change.key.startsWith("flags.OnePiece.") ) change = change.effect._prepareFlagChange(model, change);
    if ( ActiveEffect5e.FORMULA_FIELDS.has(change.key) ) {
      const field = new FormulaField({ deterministic: change.key !== "system.damageBonus" });
  ...
  _prepareFlagChange(actor, change) {
    ...
    const data = CONFIG.DND5E.characterFlags[key.replace("flags.OnePiece.", "")];
  ...
  _prepareExhaustionLevel() {
    ...
    let level = this.getFlag("onepiece-system", "exhaustionLevel");
  ...
  async createRiderConditions() {
    ...
    for ( const status of this.getFlag("onepiece-system", "riders.statuses") ?? [] ) {
      riders.add(status);
    }
  ...
  async createRiderEnchantments(options={}) {
    ...
    if ( chatMessageOrigin ) {
      ...
      profile = activity?.effects.find(e => e._id === message?.getFlag("onepiece-system", "use.enchantmentProfile"));
    ...
    for ( const id of profile.riders.activity ) {
      ...
      foundry.utils.setProperty(activityData, "flags.OnePiece.dependentOn", this.id);
    ...
    riderEffects.forEach(e => foundry.utils.setProperty(e, "flags.OnePiece.dependentOn", this.id));
      ...
      const riderItems = await Item5e.createWithContents(
        (await Promise.all(profile.riders.item.map(uuid => fromUuid(uuid)))).filter(_ => _), {
          transformAll: item => {
            ...
            foundry.utils.setProperty(itemData, "flags.OnePiece.dependentOn", this.uuid);
            foundry.utils.setProperty(itemData, "flags.OnePiece.enchantment.origin", this.uuid);
            return itemData;
          }
        }
      );
  ...
  _onUpdate(data, options, userId) {
    ...
    const newLevel = foundry.utils.getProperty(data, "flags.OnePiece.exhaustionLevel");
  ...
  static createConcentrationEffectData(activity, data={}) {
    ...
    if ( item.type === "spell" ) effectData["flags.OnePiece.spellLevel"] = item.system.level;
  ...
  static onRenderActiveEffectConfig(app, html, context) {
    const element = new foundry.data.fields.SetField(new foundry.data.fields.StringField(), {}).toFormGroup({
      ...
      name: "flags.OnePiece.riders.statuses",
      value: app.document.getFlag("onepiece-system", "riders.statuses") ?? [],
  ...
  addDependent(...dependent) {
    ...
    return Promise.all(dependent.map(d => d.setFlag("onepiece-system", "dependentOn", this.uuid))).then(() => this);
  }
  ...
  getDependents() {
    ...
    return (this.getFlag("onepiece-system", "dependents") || []).reduce((arr, { uuid }) => {
      let doc;
  ...
  async richTooltip(enrichmentOptions={}) {
    ...
    return {
      content: await foundry.applications.handlebars.renderTemplate(
        "systems/onepiece-system/templates/effects/parts/effect-tooltip.hbs", {
          effect: this,
