...
export default class TransformActivity extends ActivityMixin(BaseTransformActivityData) {
  ...
  static metadata = Object.freeze(
    foundry.utils.mergeObject(super.metadata, {
      type: "transform",
      img: "systems/onepiece-system/icons/svg/activity/transform.svg",
   ...
   */
  get canTransform() {
    return game.user.can("ACTOR_CREATE") && (game.user.isGM || game.settings.get("onepiece-system", "allowPolymorphing"));
  }
  ...
  async _finalizeMessageConfig(usageConfig, messageConfig, results) {
    ...
    if ( usageConfig.transform?.profile ) {
      foundry.utils.setProperty(messageConfig.data, "flags.OnePiece.transform.profile", usageConfig.transform.profile);
    }
  }
  ...
  async _finalizeUsage(config, results) {
    ...
    if ( profile ) {
      ...
      if ( uuid ) {
        if ( results.message instanceof ChatMessage ) results.message.setFlag("onepiece-system", "transform.uuid", uuid);
        else foundry.utils.setProperty(results.message, "flags.OnePiece.transform.uuid", uuid);
      }
    }
  ...
  static async #transformActor(event, target, message) {
    ...
    const profileId = message.getFlag("onepiece-system", "transform.profile");
    const profile = this.profiles.find(p => p._id === profileId) || this.profiles[0];
    const uuid = message.getFlag("onepiece-system", "transform.uuid") ?? await this.queryActor(profile);
