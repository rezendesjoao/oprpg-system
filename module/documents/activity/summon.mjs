...
export default class SummonActivity extends ActivityMixin(BaseSummonActivityData) {
  ...
  static metadata = Object.freeze(
    foundry.utils.mergeObject(super.metadata, {
      type: "summon",
      img: "systems/onepiece-system/icons/svg/activity/summon.svg",
   ...
   */
  get canSummon() {
    return game.user.can("TOKEN_CREATE") && (game.user.isGM || game.settings.get("onepiece-system", "allowSummoning"));
  }
  ...
  async placeSummons(options) {
    ...
    const actor = await dnd5e.documents.Actor5e.fetchExisting(summonUuid, {
      origin: { key: "flags.OnePiece.summon.origin", value: this.item?.uuid }
    });
  ...
  async getChanges(actor, profile, options) {
    ...
    // Add flags
    actorUpdates["flags.OnePiece.summon"] = {
