...
export default function ActivityMixin(Base) {
  class Activity extends DependentDocumentMixin(PseudoDocumentMixin(Base)) {
    ...
    static metadata = Object.freeze({
      ...
      usage: {
        actions: {},
        chatCard: "systems/onepiece-system/templates/chat/activity-card.hbs",
    ...
    async use(usage={}, dialog={}, message={}) {
      ...
      if ( usageConfig.concentration?.begin ) {
        const effect = await item.actor.beginConcentrating(activity, { "flags.OnePiece.scaling": usageConfig.scaling });
        if ( effect ) {
          results.effects ??= [];
      ...
      if ( usageConfig.subsequentActions !== false ) {
        ...
        if ( consumed ) item.updateSource({ "flags.OnePiece.consumed": consumed });
    ...
    _prepareUsageConfig(config) {
      ...
      const levelingFlag = this.item.getFlag("onepiece-system", "spellLevel");
      if ( levelingFlag ) {
      ...
      if ( this.requiresConcentration && !game.settings.get("onepiece-system", "disableConcentration") ) {
        ...
        if ( limit && (limit <= effects.size) ) config.concentration.end ??= effects.find(e => {
          const data = e.flags.OnePiece?.item?.data ?? {};
    ...
    async _prepareUsageScaling(usageConfig, messageConfig, item) {
      ...
      if ( usageConfig.scaling ) {
        ...
        if ( usageConfig.scaling !== item.flags.OnePiece?.scaling ) {
          item.actor._embeddedPreparation = true;
          item.updateSource({ "flags.OnePiece.scaling": usageConfig.scaling });
        ...
        if ( linkedActivity ) {
          ...
          if ( foundry.utils.getType(results) === "Object" ) {
            ...
            if ( updates.delete.includes(linkedActivity.item.id)
              && (this.item.getFlag("onepiece-system", "cachedFor") === otherLinkedActivity?.relativeUUID) ) {
    ...
    async rollDamage(config={}, dialog={}, message={}) {
      ...
      if ( canUpdate && !foundry.utils.isEmpty(lastDamageTypes)
        ...
        await this.item.setFlag("onepiece-system", `last.${this.id}.damageType`, lastDamageTypes);
      }
    ...
    async #onChatAction(event, target, message) {
      ...
      const item = (consumed || scaling) ? this.item.clone({
        "flags.OnePiece": { consumed, scaling }
    ...
    getLinkedActivity(relativeUUID) {
      ...
      relativeUUID ??= this.item.getFlag("onepiece-system", "cachedFor");
    ...
    getRollData(options) {
      ...
      rollData.consumed = this.item.flags.OnePiece?.consumed;
