...
export default class AttackActivity extends ActivityMixin(BaseAttackActivityData) {
  ...
  static metadata = Object.freeze(
    foundry.utils.mergeObject(super.metadata, {
      type: "attack",
      img: "systems/onepiece-system/icons/svg/activity/attack.svg",
  ...
  _usageChatButtons(message) {
    const buttons = [{
      ...
      icon: '<i class="dnd5e-icon" data-src="systems/onepiece-system/icons/svg/trait-weapon-proficiencies.svg" inert></i>',
      dataset: {
        action: "rollAttack"
      }
    }];
  ...
  async _triggerSubsequentActions(config, results) {
    this.rollAttack({ event: config.event }, {}, { data: { "flags.OnePiece.originatingMessage": results.message?.id } });
  }
  ...
  async rollAttack(config={}, dialog={}, message={}) {
    ...
    const rollConfig = foundry.utils.mergeObject({
      ammunition: this.item.getFlag("onepiece-system", `last.${this.id}.ammunition`),
      attackMode: this.item.getFlag("onepiece-system", `last.${this.id}.attackMode`),
      elvenAccuracy: this.actor?.getFlag("onepiece-system", "elvenAccuracy")
        && CONFIG.DND5E.characterFlags.elvenAccuracy.abilities.includes(this.ability),
      halflingLucky: this.actor?.getFlag("onepiece-system", "halflingLucky"),
      mastery: this.item.getFlag("onepiece-system", `last.${this.id}.mastery`),
    ...
    for ( const key of ["ammunition", "attackMode", "mastery"] ) {
      ...
      foundry.utils.setProperty(messageConfig.data, `flags.OnePiece.roll.${key}`, rolls[0].options[key]);
    }
    ...
    if ( canUpdate && !foundry.utils.isEmpty(flags) && (this.actor && this.actor.items.has(this.item.id)) ) {
      await this.item.setFlag("onepiece-system", `last.${this.id}`, flags);
    }
    ...
    if ( canUpdate && ammoUpdate?.destroy ) {
      ...
      await attackMessage?.setFlag("onepiece-system", "roll.ammunitionData", data);
  ...
  static #rollDamage(event, target, message) {
    ...
    const attackMode = lastAttack?.getFlag("onepiece-system", "roll.attackMode");

    // Fetch the ammunition used with the last attack roll
    let ammunition;
    const actor = lastAttack?.getAssociatedActor();
    if ( actor ) {
      const storedData = lastAttack.getFlag("onepiece-system", "roll.ammunitionData");
      ammunition = storedData
        ? new Item.implementation(storedData, { parent: actor })
        : actor.items.get(lastAttack.getFlag("onepiece-system", "roll.ammunition"));
    }
