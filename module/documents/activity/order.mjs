...
export default class OrderActivity extends ActivityMixin(BaseOrderActivityData) {
  ...
  static metadata = Object.freeze(foundry.utils.mergeObject(super.metadata, {
    type: "order",
    img: "systems/onepiece-system/icons/svg/activity/order.svg",
    title: "DND5E.FACILITY.Order.Issue",
    usage: {
      actions: {
        pay: OrderActivity.#onPayOrder
      },
      chatCard: "systems/onepiece-system/templates/chat/order-activity-card.hbs",
  ...
  _prepareUsageScaling(usageConfig, messageConfig, item) {
    ...
    messageConfig.data.flags.OnePiece.order = { costs, craft, trade };
  }
  ...
  _usageChatButtons(message) {
    const { costs } = message.data.flags.OnePiece.order;
  ...
  async _usageChatContext(message) {
    const { costs, craft, trade } = message.data.flags.OnePiece.order;
  ...
  static async #onPayOrder(event, target, message) {
    ...
    const order = message.getFlag("onepiece-system", "order");
    const config = foundry.utils.expandObject({ "data.flags.OnePiece.order": order });
    if ( method === "automatic" ) {
      try {
    ...
    foundry.utils.setProperty(config, "data.flags.OnePiece.order.costs.paid", true);
