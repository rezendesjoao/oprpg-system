...
/**
 * Register package source books from `flags.OnePiece.sourceBooks`.
...
function registerSourceBooks(manifest) {
  if ( !manifest.flags.OnePiece?.sourceBooks ) return;
  Object.assign(CONFIG.DND5E.sourceBooks, manifest.flags.OnePiece.sourceBooks);
  return "source books";
}
...
/**
 * Register package spell lists from `flags.OnePiece.spellLists`.
 * @param {Module|System|World} manifest  Manifest from which to register data.
 * @returns {string|void}                 Description of the data registered.
 */
function registerSpellLists(manifest) {
  if ( !Array.isArray(manifest.flags.OnePiece?.spellLists) ) return;
  manifest.flags.OnePiece.spellLists.forEach(uuid => dnd5e.registry.spellLists.register(uuid));
  return "spell lists";
}
...
/**
 * Set application based on `flags.OnePiece.display`.
 * @param {Compendium} pack  Pack to set up.
 * @returns {string|void}    Description of the step.
 */
function setupPackDisplay(pack) {
  const display = pack.metadata.flags.display ?? pack.metadata.flags.OnePiece?.display;
...
/**
 * Set default sorting order based on `flags.OnePiece.sorting`.
 * @param {Compendium} pack  Pack to set up.
 * @returns {string|void}    Description of the step.
 */
function setupPackSorting(pack) {
  collectionSortingModes ??= game.settings.get("core", "collectionSortingModes") ?? {};
  if ( !pack.metadata.flags.OnePiece?.sorting || collectionSortingModes[pack.metadata.id] ) return;
  collectionSortingModes[pack.metadata.id] = pack.metadata.flags.OnePiece.sorting;
  sortingChanged = true;
