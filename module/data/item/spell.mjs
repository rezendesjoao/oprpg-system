...
export default class SpellData extends ItemDataModel.mixin(ActivitiesTemplate, ItemDescriptionTemplate) {
  ...
  get criticalThreshold() {
    return this.parent?.actor?.flags.OnePiece?.spellCriticalThreshold ?? Infinity;
  }
  ...
  get linkedActivity() {
    ...
    const uuid = this.parent.getFlag("onepiece-system", "cachedFor");
    ...
    // return fromUuidSync(this.parent.getFlag("onepiece-system", "cachedFor"), { relative, strict: false }) ?? null;
  }
  ...
  static _migrateComponentData(source) {
    ...
    if ( components.length ) {
      foundry.utils.setProperty(source, "flags.OnePiece.migratedProperties", components);
    }
  }
    ...
    if ( this.parent.actor ) {
      ...
      if ( sourceItem?.type !== "class" ) {
        ...
        if ( !grantingItem ) {
          ...
          const advancementOrigin = this.parent.flags?.OnePiece?.advancementOrigin;
  ...
  getRollData(...options) {
    ...
    data.item.level = data.item.level + (this.parent.getFlag("onepiece-system", "scaling")
