...
export default class ActivitiesTemplate extends SystemDataModel {
  ...
  static initializeActivities(source) {
    ...
    if ( source._id && source.type && ("value" in uses) && uses.max ) {
      foundry.utils.setProperty(source, "flags.OnePiece.migratedUses", uses.value);
    }
  }
  ...
  async recoverUses(periods, rollData) {
    const updates = {};
    const rolls = [];
    const autoRecharge = game.settings.get("onepiece-system", "autoRecharge");
  ...
  async preUpdateActivities(changed, options, user) {
    ...
    if ( !riders.activity.size && !riders.effect.size ) {
      foundry.utils.setProperty(changed, "flags.OnePiece.-=riders", null);
    } else {
      foundry.utils.setProperty(changed, "flags.OnePiece.riders", Object.entries(riders)
