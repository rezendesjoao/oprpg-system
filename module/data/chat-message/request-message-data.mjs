...
export default class RequestMessageData extends ChatMessageDataModel {
  ...
  static metadata = Object.freeze(foundry.utils.mergeObject(super.metadata, {
    ...
    template: "systems/onepiece-system/templates/chat/request-card.hbs"
  }, { inplace: false }));
  ...
  static async #handleRequest(event, target) {
    ...
    if ( (result instanceof ChatMessage) && !result.getFlag("onepiece-system", "requestResult") ) {
      return result.setFlag("onepiece-system", "requestResult", { actorUuid: actor.uuid, requestId: this.parent.id });
    }
  }
  ...
  static onCreateMessage(message) {
    const flag = message.getFlag("onepiece-system", "requestResult");
  ...
  static onUpdateResultMessage(message, changes, options, userId) {
    const flag = foundry.utils.getProperty(changes, "flags.OnePiece.requestResult");
    if ( flag && (game.users.activeGM === game.user) ) RequestMessageData.#updateRequestTargets(message, flag);
  }
