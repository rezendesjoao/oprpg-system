...
export default class NPCData extends CreatureTemplate {
  ...
  async resistSave(message) {
    ...
    if ( message.flags.OnePiece?.roll?.type !== "save" ) throw new Error("Chat message must contain a save roll.");
    if ( message.flags.OnePiece?.roll?.forceSuccess ) throw new Error("Save has already been resisted.");
    await this.parent.update({ "system.resources.legres.spent": this.resources.legres.spent + 1 });
    await message.setFlag("onepiece-system", "roll.forceSuccess", true);
  }
  ...
  async toEmbed(config, options={}) {
    ...
    template.innerHTML = await foundry.applications.handlebars.renderTemplate(
      "systems/onepiece-system/templates/actors/embeds/npc-embed.hbs", context
    );
  ...
  async _prepareEmbedContext(rulesVersion) {
    ...
    const o = this.parent.flags.OnePiece?.statBlockOverride ?? {};
