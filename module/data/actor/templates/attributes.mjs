...
export default class AttributesFields {
  ...
  static prepareEncumbrance(rollData, { validateItem }={}) {
    ...
    if ( game.settings.get("onepiece-system", "currencyWeight") && currency ) {
    ...
    const sizeConfig = CONFIG.DND5E.actorSizes[
      keys[this.parent.flags.OnePiece?.powerfulBuild ? Math.min(index + 1, keys.length - 1) : index]
    ];
  ...
  static prepareExhaustionLevel() {
    ...
    const level = exhaustion?.getFlag("onepiece-system", "exhaustionLevel");
  ...
  static prepareInitiative(rollData) {
    ...
    const flags = this.parent.flags.OnePiece ?? {};
  ...
  static async onUpdateHP(changed, options, userId) {
    ...
    if ( !game.settings.get("onepiece-system", "disableConcentration") && (userId === game.userId)
