...
export default class CommonTemplate extends ActorDataModel.mixin(CurrencyTemplate) {
  ...
  calculateAbilityCheckProficiency(multiplier, ability, options={}) {
    let roundDown = true;
    if ( (multiplier < 1) && ((game.settings.get("onepiece-system", "rulesVersion") === "legacy") || options.skill) ) {
      if ( this.parent._isRemarkableAthlete(ability) ) {
        multiplier = .5;
        roundDown = false;
      }
      else if ( this.parent.flags.OnePiece?.jackOfAllTrades ) multiplier = .5;
    }
  ...
  calculateToolProficiency(multiplier, ability, options={}) {
    if ( (multiplier === 1) && this.parent.flags.OnePiece?.toolExpertise ) {
