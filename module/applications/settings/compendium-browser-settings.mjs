...
export default class CompendiumBrowserSettingsConfig extends Application5e {
  ...
  static PARTS = {
    sidebar: {
      id: "sidebar",
      template: "systems/onepiece-system/templates/compendium/sources-sidebar.hbs"
    },
    packs: {
      id: "packs",
      template: "systems/onepiece-system/templates/compendium/sources-packs.hbs"
    }
  };
  ...
  _preparePackGroupContext(documentType, packs, sources) {
    ...
    return {
      ...
      entries: Array.from(packs.map(id => {
        ...
        if ( packageName === "onepiece-system" ) {
  ...
  async _onToggleSource(target) {
    ...
    const setting = { ...game.settings.get("onepiece-system", "packSourceConfiguration"), ...packs };
    await game.settings.set("onepiece-system", "packSourceConfiguration", setting);
    this.render();
  }
  ...
  static collateSources() {
    ...
    const setting = game.settings.get("onepiece-system", "packSourceConfiguration");
