...
export default class CalendarSettingsConfig extends BaseSettingsConfig {
  ...
  async _prepareConfigContext(context, options) {
    const data = game.settings.get("onepiece-system", "calendarConfig");
  ...
  async _preparePreferencesContext(context, options) {
    const data = game.settings.get("onepiece-system", "calendarPreferences");
    ...
    } else if ( !game.settings.get("onepiece-system", "calendarConfig")?.enabled ) {
