...
export default class Award extends Application5e {
  ...
  static PARTS = {
    award: {
      template: "systems/onepiece-system/templates/apps/award.hbs"
    }
  };
  ...
  async _prepareContext(options) {
    ...
    context.hideXP = game.settings.get("onepiece-system", "levelingMode") === "noxp";
  ...
  static prepareDestinations(destinations, savedDestinations) {
    const icons = {
      container: '<dnd5e-icon class="fa-fw" src="systems/onepiece-system/icons/svg/backpack.svg"></dnd5e-icon>',
  ...
  _saveDestinations(destinations) {
    ...
    target.setFlag("onepiece-system", "awardDestinations", destinations);
  }
  ...
  static async handleAward(message) {
    ...
    try {
      ...
      // Otherwise show the UI with defaults
        ...
        const savedDestinations = game.user.getFlag("onepiece-system", "awardDestinations");
