...
export default class ItemSheet5e extends PrimarySheetMixin(DocumentSheet5e) {
  ...
  static PARTS = {
    header: {
      template: "systems/onepiece-system/templates/items/header.hbs"
    },
    tabs: {
    ...
    activities: {
      template: "systems/onepiece-system/templates/items/activities.hbs",
      templates: [
        "systems/onepiece-system/templates/inventory/columns/uses.hbs",
        "systems/onepiece-system/templates/shared/activities.hbs"
      ],
      scrollable: [""]
    },
    advancement: {
      template: "systems/onepiece-system/templates/items/advancement.hbs",
      scrollable: [""]
    },
    description: {
      template: "systems/onepiece-system/templates/items/description.hbs",
      scrollable: [""]
    },
    details: {
      template: "systems/onepiece-system/templates/items/details.hbs",
      scrollable: [""]
    },
    effects: {
      template: "systems/onepiece-system/templates/items/effects.hbs",
      scrollable: [""]
    }
  };
  ...
  async _prepareEffectsContext(context, options) {
    const effectMap = {};
    const riders = [];
    const riderIds = new Set(this.item.getFlag("onepiece-system", "riders.effect") ?? []);
  ...
  _getAdvancementTags(advancement) {
    ...
    if ( advancement.classRestriction === "primary" ) {
      tags.push({
        ...
        icon: "systems/onepiece-system/icons/svg/original-class.svg"
      });
    } else if ( advancement.classRestriction === "secondary" ) {
      tags.push({
        label: "DND5E.AdvancementClassRestrictionSecondary",
        icon: "systems/onepiece-system/icons/svg/multiclass.svg"
      });
    }
    return tags;
  }
  ...
  async _onDropAdvancement(event, data) {
    ...
    if ( this.item.actor?.system.metadata?.supportsAdvancement && !game.settings.get("onepiece-system", "disableAdvancements") ) {
