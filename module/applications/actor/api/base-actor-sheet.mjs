...
import CursedEnergyConfig from "../config/cursed-energy-config.mjs";
import CursedEnergyDiceConfig from "../config/cursed-energy-dice-config.mjs";
...
export default class BaseActorSheet extends PrimarySheetMixin(
  ...
  constructor(options={}) {
    ...
    const { width, height } = game.user.getFlag("onepiece-system", `sheetPrefs.${key}`) ?? {};
    options.position ??= {};
  ...
  static LIMITED_PARTS = {
    header: {
      template: "systems/onepiece-system/templates/actors/limited-header.hbs"
    },
    biography: {
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/onepiece-system/templates/actors/limited-body.hbs",
      scrollable: [""]
    }
  };
  ...
  async _prepareContext(options) {
    const context = {
      ...
      sidebarCollapsed: !!game.user.getFlag("onepiece-system", this._sidebarCollapsedKeyPath),
  ...
  async _prepareSpecialTraitsContext(context, options) {
    ...
    for ( const [key, config] of Object.entries(CONFIG.DND5E.characterFlags) ) {
      const flag = { ...config, name: `flags.OnePiece.${key}`, value: foundry.utils.getProperty(flags.data, key) };
  ...
  _prepareSpellbook(context) {
    ...
    (context.itemCategories.spells ?? []).forEach(spell => {
      ...
      // Spells from items
      if ( spell.getFlag("onepiece-system", "cachedFor") ) {
        method = "item";
  ...
  _prepareItem(item, ctx) {
    ...
    // Linked Uses
    const cachedFor = fromUuidSync(item.flags.OnePiece?.cachedFor, { relative: item.parent, strict: false });
    if ( cachedFor ) {
  ...
  _renderSpellbook(context, options) {
    for ( const { usesSlots, pips, slot, dataset } of Object.values(context.spellbook) ) {
      ...
      pips.forEach(({ classes, n, tooltip, label, filled}) => {
        ...
        const icon = '<dnd5e-icon src="systems/onepiece-system/icons/svg/spell-slot.svg"></dnd5e-icon>';
  ...
  async _onRender(context, options) {
    ...
    if ( this.tabGroups.primary ) {
      const sidebarCollapsed = !!game.user.getFlag("onepiece-system", this._sidebarCollapsedKeyPath);
  ...
  async #changeLevel(event) {
    ...
    if ( !game.settings.get("onepiece-system", "disableAdvancements") ) {
  ...
  #saveSheetSize(position) {
    ...
    game.user.setFlag("onepiece-system", `sheetPrefs.${key}`, prefs);
  }
  ...
  static #showConfiguration(event, target) {
    ...
    switch ( target.dataset.config ) {
      ...
      case "cursedEnergy":
        return this._renderChild(new CursedEnergyConfig(config));
      case "cursedEnergyDice":
        return this._renderChild(new CursedEnergyDiceConfig(config));
      case "initiative":
  ...
  static #toggleSidebar(event, target) {
    ...
    game.user.setFlag("onepiece-system", this._sidebarCollapsedKeyPath, collapsed);
  }
  ...
  _processFormData(event, form, formData) {
    ...
    for ( const [key, value] of Object.entries(submitData.flags?.dnd5e ?? {}) ) {
      ...
      if ( this.actor.isToken && this.actor.parent.baseActor.getFlag("onepiece-system", key) ) continue;

      delete submitData.flags.OnePiece[key];
      if ( foundry.utils.hasProperty(this.document._source, `flags.OnePiece.${key}`) ) {
        submitData.flags.OnePiece[`-=${key}`] = null;
      }
    }
  ...
  async _onDropActor(event, actor) {
    const canPolymorph = game.user.isGM || (this.actor.isOwner && game.settings.get("onepiece-system", "allowPolymorphing"));
    ...
    const settings = await TransformDialog.promptSettings(this.actor, actor, {
      transform: { settings: game.settings.get("onepiece-system", "transformationSettings") },
      windowId: this.window?.windowId
    });
    if ( !settings ) return;
    await game.settings.set("onepiece-system", "transformationSettings", settings.toObject());
  ...
  async _onDropCreateItems(event, items, behavior) {
    ...
    if ( multipleAdvancements && !game.settings.get("onepiece-system", "disableAdvancements") ) {
  ...
  async _onDropSingleItem(event, itemData, { container=null }={}) {
    ...
    if ( actor.system.metadata?.supportsAdvancement && !foundry.utils.isEmpty(itemData.system.advancement)
        && !game.settings.get("onepiece-system", "disableAdvancements") ) {
