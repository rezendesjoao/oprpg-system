...
export default class BaseRestDialog extends Dialog5e {
  ...
  static DEFAULT_OPTIONS = {
    ...
    templates: [
      "systems/onepiece-system/templates/actors/rest/parts/hit-dice.hbs",
      "systems/onepiece-system/templates/actors/rest/parts/rest-request.hbs"
    ]
  };
  ...
  static PARTS = {
    ...super.PARTS,
    content: {
      template: "systems/onepiece-system/templates/actors/rest/base-rest.hbs"
    }
  };
  ...
  get promptNewDay() {
    const duration = CONFIG.DND5E.restTypes[this.config.type]
      ?.duration?.[game.settings.get("onepiece-system", "restVariant")] ?? 0;
  ...
  async _prepareContext(options) {
    const context = {
      ...
      variant: game.settings.get("onepiece-system", "restVariant")
    };
    ...
    if ( this.isPartyGroup ) {
      const restSettings = this.actor.getFlag("onepiece-system", "restSettings") ?? {};
      context.request = [
        {
          field: new BooleanField({
  ...
  static async #handleFormSubmission(event, form, formData) {
    ...
    if ( this.isPartyGroup ) {
      ...
      this.actor.setFlag("onepiece-system", "restSettings", data);
    }
