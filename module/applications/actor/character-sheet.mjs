...
export default class CharacterActorSheet extends BaseActorSheet {
  ...
  static PARTS = {
    header: {
      template: "systems/onepiece-system/templates/actors/character-header.hbs"
    },
    sidebar: {
      container: { classes: ["main-content"], id: "main" },
      template: "systems/onepiece-system/templates/actors/character-sidebar.hbs"
    },
    details: {
      classes: ["col-2"],
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/onepiece-system/templates/actors/tabs/character-details.hbs",
      scrollable: [""]
    },
    inventory: {
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/onepiece-system/templates/actors/tabs/character-inventory.hbs",
      templates: [
        "systems/onepiece-system/templates/inventory/inventory.hbs", "systems/onepiece-system/templates/inventory/activity.hbs",
        "systems/onepiece-system/templates/inventory/encumbrance.hbs", "systems/onepiece-system/templates/inventory/containers.hbs"
      ],
      scrollable: [""]
    },
    features: {
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/onepiece-system/templates/actors/tabs/character-features.hbs",
    ...
    biography: {
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/onepiece-system/templates/actors/tabs/character-biography.hbs",
      scrollable: [""]
    },
    bastion: {
      container: { classes: ["tab-body"], id: "tabs" },
      template: "systems/onepiece-system/templates/actors/tabs/character-bastion.hbs",
      scrollable: [""]
    },
    // specialTraits: {
    //   classes: ["flexcol"],
...
context.dominioExpandido = this.actor.getFlag("onepiece-system", "dominioExpandido") === true;
    ...
    context.seisOlhosMode = this.actor.getFlag("onepiece-system", "seisOlhosMode") ?? "sealed";

    // Fulgor Negro
    context.fulgorNegro    = !!this.actor.system.manipulation?.abilities?.fulgorNegro?.unlocked;
    context.fulgorPrimaria  = this.actor.getFlag("onepiece-system", "fulgorPrimaria")  ?? 20;
    ...
    context.fulgorZona      = this.actor.getFlag("onepiece-system", "fulgorZona") ?? false;
...
async _onRender(context, options) {
  ...
  if ( !this.actor.limited ) {
    ...
    this.element.querySelectorAll("input[name='flags.onepiece-system.seisOlhosMode']")
      .forEach(radio => radio.addEventListener("change", async (event) => {
        ...
        await this.actor.setFlag("onepiece-system", "seisOlhosMode", mode);
  ...
  static hasBastion(actor) {
    ...
    return game.settings.get("onepiece-system", "bastionConfiguration")?.enabled && (actor.system.details.level >= threshold);
  }

  /* -------------------------------------------- */

  /**
   * Prepara o contexto da aba "Akuma no Mi" (One Piece).
   */
  ...
  /**
   * Prepara o contexto para a aba de Manipulação de Energia (Skill Tree — legado JJ).
   */
