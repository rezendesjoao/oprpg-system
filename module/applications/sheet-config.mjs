...
export default class SheetConfig5e extends foundry.applications.apps.DocumentSheetConfig {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      template: "systems/onepiece-system/templates/shared/sheet-config.hbs"
    });
  }
  ...
  async _updateObject(event, formData) {
    ...
    if ( "flags.OnePiece.theme" in formData ) {
      const sheet = this.object.sheet.element?.[0];
      if ( sheet ) setTheme(sheet, formData["flags.OnePiece.theme"]);
    }
  }
}
