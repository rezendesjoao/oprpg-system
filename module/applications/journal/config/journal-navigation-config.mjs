...
export default class JournalNavigationConfig extends DocumentSheet5e {
  ...
  static PARTS = {
    form: {
      template: "systems/onepiece-system/templates/journal/config/navigation-config.hbs"
    }
  };
  ...
  async _prepareContext(options) {
    ...
    const data = this.document.getFlag("onepiece-system", "navigation") ?? {};
    ...
    context.fields = ["previous", "up", "next"].map(name => ({
      ...
      name: `flags.OnePiece.navigation.${name}`,
  ...
  _processFormData(event, form, formData) {
    ...
    const navigation = submitData.flags.OnePiece.navigation;
    const keys = Object.keys(this.document.flags.OnePiece ?? {});
    if ( Object.values(navigation).some(v => v) ) {
      submitData.flags.OnePiece.navigation = Object.entries(navigation).reduce((obj, [k, v]) => {
        if ( v ) obj[k] = v;
        else obj[`-=${k}`] = null;
        return obj;
      }, {});
