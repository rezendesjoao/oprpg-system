...
export default class OrderUsageDialog extends ActivityUsageDialog {
  ...
  static PARTS = {
    order: {
      template: "systems/onepiece-system/templates/activity/order-usage.hbs"
    },
    footer: {
  ...
  _prepareCostsContext(context, { days, gold }) {
    const { duration } = game.settings.get("onepiece-system", "bastionConfiguration");
    context.costs = {
      days: {
  ...
  async _prepareOrderContext(context, options) {
    ...
    let { duration } = game.settings.get("onepiece-system", "bastionConfiguration");
