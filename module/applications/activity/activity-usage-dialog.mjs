...
export default class ActivityUsageDialog extends Dialog5e {
  ...
  static PARTS = {
    scaling: {
      template: "systems/onepiece-system/templates/activity/activity-usage-scaling.hbs"
    },
    concentration: {
      template: "systems/onepiece-system/templates/activity/activity-usage-concentration.hbs"
    },
    consumption: {
      template: "systems/onepiece-system/templates/activity/activity-usage-consumption.hbs"
    },
    creation: {
      template: "systems/onepiece-system/templates/activity/activity-usage-creation.hbs"
    },
    footer: {
  ...
  async _prepareContext(options) {
    if ( "scaling" in this.config ) {
      this.#item = this.#item.clone({ "flags.OnePiece.scaling": this.config.scaling }, { keepId: true });
    }
    return {
  ...
  async _prepareConcentrationContext(context, options) {
    if ( !this.activity.requiresConcentration || game.settings.get("onepiece-system", "disableConcentration")
